...
class Util {
    ...
    //"Chủ Nhật", "Thứ Hai", "Thứ Ba", "Thứ Tư", "Thứ Năm", "Thứ Sáu", "Thứ Bảy"
   	public function namedayendtovi($dayenglish)
	{
		switch ($dayenglish) {
    ...
    function ConvertDateXuoi($date_str) {
        if(!$date_str) return "";
        $tmp=explode("/",$date_str);
        if($tmp[0]<10)$tmp[0]="0".abs($tmp[0]);
        if($tmp[1]<10)$tmp[1]="0".abs($tmp[1]);
        $ket_qua=$tmp[2]."-".$tmp[1]."-".$tmp[0];
        return $ket_qua;
    }
 //Tu MYSQL sang VietNam
    function ConvertDateTimeToView($date_str) {
        if(!$date_str) return "";
        
        $tmp=explode("-",$date_str);
        if($tmp[2]<10)$tmp[2]="0".abs($tmp[2]);
        if($tmp[1]<10)$tmp[1]="0".abs($tmp[1]);
        $ket_qua=$tmp[2]."/".$tmp[1]."/".$tmp[0];
        return $ket_qua;
    }
